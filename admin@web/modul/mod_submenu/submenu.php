...
<input type=button class='btn btn-primary btn' value='Tambah Sub Menu' onclick=\"window.location.href='?module=submenu&act=tambahsubmenu';\">
    ...
    $tampil = mysqli_query($koneksi,"SELECT * FROM submenu,menuutama WHERE submenu.id_main=menuutama.id_main");
  
    $no = 1;
    while($r=mysqli_fetch_array($tampil)){
	if($r['id_submain']!=0){
		$sub = mysqli_fetch_array(mysql_query($koneksi,"SELECT * FROM submenu WHERE id_sub=$r[id_submain]"));
      ...
      echo "<tr><td class='left' width='25'>$no</td>
                ...
                <td class='center'>$r[aktif]</td>               
		            <td align='center'>
										<a href='?module=submenu&act=editsubmenu&id=$r[id_sub]'>
										<img src=img/edit.png></a> &nbsp;
									<a href='$aksi?module=submenu&act=hapus&id=$r[id_sub]'>
                                 ...
                                 <div class='form-group'>
                                        ...
                                        <div class='form-group'>
                                            ...
                                            <select name='menu_utama' class='form-control'>
                                                ...
                                                $tampil=mysqli_query($koneksi,"SELECT * FROM menuutama WHERE aktif='Y' ORDER BY nama_menu");
            while($r=mysqli_fetch_array($tampil)){
    ...
    echo "</select>
     <div class='form-group'>
                                            ...
                                            <select name='sub_menu' class='form-control'>
                                                ...
                                                $tampil=mysqli_query("SELECT * FROM menuutama WHERE aktif='Y' ORDER BY nama_menu");
            while($r=mysqli_fetch_array($tampil)){
              echo "<option value=$r[id_main]>$r[nama_menu]</option>";
            }
    echo "</select>
  ...
  case "editsubmenu":
    ...
    echo "</select>
     <div class='form-group'>
                                            ...
                                            <select name='sub_menu' class='form-control'>
                                                <option value=0 selected>- Pilih Sub Menu Dari Menu Utama -</option>";
                                                $tampil2=mysqli_query("SELECT * FROM submenu WHERE id_submain=0 AND aktif='Y' ORDER BY nama_sub");
          if ($r[id_submain]==0){
